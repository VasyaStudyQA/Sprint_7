...
class TestCreateCourier:
    ...
    def test_creating_courier_full_data_success(self, courier_cleanup):
        courier_data, create_json, create_status = CourierMethods().create_courier()
        courier_cleanup['courier_data'] = courier_data
        assert create_json == {'ok': True}
    ...
    def test_creating_courier_used_data_error(self, courier_no_auth):
        used_data = courier_no_auth["data"].copy()
        create_json, create_status = CourierMethods().create_courier(used_data)[1:]
        assert create_json['message'] == "Этот логин уже используется. Попробуйте другой."
    ...
    def test_creating_courier_half_data_error(self, courier_data):
        ...
        assert create_json['message'] == ErrorMessages.half_create_courier_data 
