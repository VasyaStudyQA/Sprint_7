...
class TestOrderByTrack:
    ...
    def test_get_order_by_track_full_data_success(self, order):
        get_order_json, get_order_status = OrderMethods().get_order_by_track(order['order_track'])
        assert get_order_json['order'] 
        assert get_order_status == 200       
    ...
    def test_get_order_by_track_no_data_error(self):
        ...
        assert get_order_json['message'] == 'Недостаточно данных для поиска' 
        assert get_order_status == 400
    ...
    def test_get_order_by_track_wrong_data_error(self):
        ...
        assert get_order_json['message'] == 'Заказ не найден' 
        assert get_order_status == 404